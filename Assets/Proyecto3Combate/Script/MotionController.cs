...
namespace InClass
{
    ...
    public class MotionController : MonoBehaviour
    {
        ...
        [SerializeField] private VectorDampener motionVectorDampener;

        public void OnMove(CallbackContext ctx)
        {
            Vector2 input = ctx.ReadValue<Vector2>();
            motionVectorDampener.TargetValue = input;
        }
        ...
        public void OnDodge(CallbackContext ctx)
        {
            ...
            {
                Vector2 motionTarget = motionVectorDampener.TargetValue;
        ...
        private void Update()
        {
            ...
            motionVector = cameraForward * motionVectorDampener.CurrentValue.y +
                           cameraTransform.right * motionVectorDampener.CurrentValue.x;
            if (!animator.GetBool("Dodging"))
            {
                if (character.State.IsLocked)
                {
                    animator.SetFloat("MotionX", motionVectorDampener.CurrentValue.x);
                    animator.SetFloat("MotionY", motionVectorDampener.CurrentValue.y);
                    transform.rotation = Quaternion.LookRotation(cameraForward, Vector3.up);
                }
                else
                {
                    animator.SetFloat("MotionY", motionVectorDampener.CurrentValue.magnitude);
